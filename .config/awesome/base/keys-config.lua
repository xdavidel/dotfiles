...
function hotkeys:init(args)
	...
	-- 	},
	-- 	{
	-- 		{ env.mod }, "j", function () heawful.tag.incmwfact(-0.05) end,
	...
	self.raw.root = {
		...
		},
		{
	 	...
	 	{
	 		{ env.mod, env.alt }, "Left", function () awful.tag.incmwfact(-0.05) end,
	 		{ description = "Decrease master width factor", group = "Layout" }
	 	},
	 	{
		...
		{
			{ env.mod, "Shift" }, "h", awful.tag.viewprev,
		...
		--},
		{
			{ env.mod, "Shift" }, "j", function() awful.layout.inc(1) end,
			{ description = "Select next layout", group = "Layouts" }
		},
		{
			{ env.mod, "Shift" }, "k", function() awful.layout.inc(-1) end,
